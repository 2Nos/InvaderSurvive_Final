...
public abstract class PlayerMainState
{
    ...
    // 추가: 기본은 None 반환
    protected virtual LocomotionStateFlags LocomotionFlag => LocomotionStateFlags.None;
    protected virtual CombatStateFlags CombatFlag => CombatStateFlags.None;
    protected virtual InteractionStateFlags InteractionFlag => InteractionStateFlags.None;
    ...
    /// </summary>
    protected bool IsLocomotion(LocomotionStateFlags state) => m_flagManager.LocomotionState.HasFlag(state);
    protected bool IsCombat(CombatStateFlags state) => m_flagManager.CombatState.HasFlag(state);
    protected bool IsInteraction(InteractionStateFlags state) => m_flagManager.InteractionState.HasFlag(state);
}
